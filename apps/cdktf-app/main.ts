...
stages.forEach((stage) => {
  ...
  const backendStack = new BackendStack(app, `cdktf-backend-${stage}`, { stage, backendBucket: preReqStack.bucketName });
  new FrontendStack(app, `cdktf-frontend-${stage}`, { stage, backendBucket: preReqStack.bucketName, apiUrl: backendStack.apiUrl });
});
